...
class ContentExtractor:
    ...
    def get_post_time_stamp(cls,post):
        ...
        timestamp = ''
        try:
            date_content = post.find_element_by_css_selector("span[id*='jsc']  > span:nth-of-type(2) > span > a > span").text
            if len(date_content ) < 6:
                if re.search(r"\d+\s*hrs|\d+\s*hr",date_content):
                    ...
                    yr = date.strftime("%Y")
                    day = date.strftime("%d")
                    timestamp = f"{day}-{month}-{yr}-{hr}:00"
                ...
                date_content = post.find_element_by_css_selector("span[id*='jsc'] > span:nth-of-type(2) > span > a > span").text
                month_reg = r"January\s*\d+|February\s*\d+|March\s*\d+|April\s*\d+|May\s*\d+|June\s*\d+|July\s*\d+|August\s*\d+|September\s*\d+|October\s*\d+|November\s*\d+|December\s*\d+"
                ...
                if re.search(r"\d:\d\d AM|\d:\d\d PM",date_content):
                    hr = re.search(r"\d|\d\d",re.search(r"\d:\d\d AM|\d:\d\d PM",date_content).group()).group()
                    mins = re.search(r":\d+",date_content).group()
                    am_pm = re.search(r"AM|PM",date_content).group()
                    if am_pm == "PM":
                        hr = int(hr) + 12
                timestamp = f"{day}-{month}-{year}-{hr}:{mins}"
                print("\n")
                print("timestamp --------------------> ",timestamp)
                time.sleep(1)
                return timestamp
    ...
    def get_like_count(cls,post):
        try:
            likes=post.find_element_by_css_selector("span.gpro0wi8.cwj9ozl2.bzsjyuwj.ja2t1vim > span > span").text
